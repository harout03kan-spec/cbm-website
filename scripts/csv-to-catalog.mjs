// Converts scripts/catalog_source.csv → src/data/catalog.ts (Product[]).
//
// Why: the live WordPress REST API (wholesaleasic.com/wp-json/cbtc/v1/products)
// is currently behind a SiteGround anti-bot challenge that returns captcha HTML
// instead of JSON, so the storefront cannot read the catalog over the network.
// This script bakes the real exported catalog into a typed data source.
//
...
//
// Run from the project root:  node scripts/csv-to-catalog.mjs
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const SRC = path.join(__dirname, 'catalog_source.csv');
...
const banner =
  ...
  '// Source of truth: scripts/catalog_source.csv (real exported catalog).\n' +
  '// Regenerate with:  node scripts/csv-to-catalog.mjs\n';
